...
def main():
    ...
    if (input_text and submit_search) or example_one or example_two or example_three:
        ...
        if example_one:
            returned_books = get_top_N_books('cozy mystery with a woman amateur sleuth', df, 5)

        elif example_two:
            returned_books = get_top_N_books('a scifi where aliens make contact with humans', df, 5)


        elif example_three:
            returned_books = get_top_N_books('literary fiction set in new york city in the early 1900s', df, 5)


        else:
            returned_books = get_top_N_books(input_text, df, 5)
